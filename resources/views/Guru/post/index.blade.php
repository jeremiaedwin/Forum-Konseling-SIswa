...
<div class="card content-card header-index-post">
    ...
    <form action="/guru/post/sortir" method="post">
    {{csrf_field()}}
...
<!-- list post -->
    ...
    <div class="card content-card daftar-post">
        <div class="card-header background-index-head" style="background-color: white; color: #3A4058;border-top: 2px solid #7D7AAE">
            <div class="media">
                ...
                <div class="media-body">
                    <h5 class="mt-3" style="font-weight: 700">{{$p->user->name}} @if($p->solved != 1) <button disabled="disabled" class="btn btn-danger btn-sm">Not Solved</button> @else ($p->solved != 1) <button disabled="disabled" class="btn btn-success btn-sm">Solved</button> @endif</h5>
                </div>
                @else
                <img class="mr-3" src="{{ asset('image/user.png') }}" style="width:64px; height:64px; border-radius:50%" alt="Generic placeholder image">
                <div class="media-body">
                    <h5 class="mt-3" style="font-weight: 700">{{$p->user->name}} Bertanya Sebagai Anonim @if($p->solved != 1) <button disabled="disabled" class="btn btn-danger btn-sm">Not Solved</button> @else ($p->solved != 1) <button disabled="disabled" class="btn btn-success btn-sm">Solved</button> @endif</h5>
                </div>
                @endif
                <!--Laporkan-->
        ...
        @if($p->image == true)
            ...
            <form  method="post" action="/guru/post/updateView/{{$p->slug}}">
            {{csrf_field()}}
            ...
            <div class="modal-content">
                ...
                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                    ...
                    <div class="modal-body">
                        <form action="/guru/report/store" method="POST">
                            {{csrf_field()}}
