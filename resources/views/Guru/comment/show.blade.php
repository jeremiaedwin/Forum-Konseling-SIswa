<div class="card card-content"  style="">
    <div class="card-body" id="balasan2">
        ...
        @foreach($r_comment as $rc)
            ...
            <div class="media mb-2" id="balasan-{{$rc->id}}" style="background-color:white; padding:10px; border-style: outset; border-bottom: 3px solid #7D7AAE;border-right: 3px solid #7D7AAE">
            @if($rc->anonymous == 1)
            <img src="{{ asset('image/user.png') }}" alt="" width="64" height="64" class="align-self-start mr-3" style="border-radius:50%">
            @else
            <img src="{{ asset('image/'. $rc->user->profile->foto_profil) }}" alt="" width="64" height="64" class="align-self-start mr-3" style="border-radius:50%; object-fit: contain;">
            @endif
                <div class="media-body" style="color:#3A4058">
                  @if(Auth::user()->id == $rc->user_id)
                      ...
                      <a id="edit-balas-komentar" data-toggle="modal" data-id="{{$rc->id}}" data-comment="{{$rc->comment}}" data-comment_id="{{$rc->comment_id}}" data-target="#editBalasanKomentar" style="float:right"><button class="btn btn-sm">edit komentar</button></a>
                  @else
                  @endif  
                    
                    @if($rc->anonymous == 1)
                    <h5 class="mt-0"><b>{{$rc->user->name}} Berkomentar Sebagai Anonymous</b></h5>
                    @else
    ...
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-body">
            ...
            <label for="">Edit Komentar</label>
                ...
                <input type="hidden" id="balasKomentarIdKomen">
