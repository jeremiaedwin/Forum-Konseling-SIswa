...
<div class="card content-card" id="content-post" style="width:95%; margin-left:auto; margin-right:auto">
    ...
    <div class="card-header background-index-head" style="background-color: white; color: #3A4058;border-top: 2px solid #7D7AAE">
        <div class="media">
            ...
            <img class="align-self-start mr-3" src="{{ asset('image/'. $posts->user->profile->foto_profil) }}" width="64" height="64"style="border-radius:50%; object-fit: cover;" alt="Generic placeholder image">
            <div class="media-body">
                <h5 class="mt-3" style="font-weight: 700">{{$posts->user->username}}</h5>
            </div>
            @else
...
<div class="mb-2 mt-2 ml-2" style="margin-left:auto; margin-right:auto"><a href="/post" class="btn btn-lg" style="border: none; color:grey; font-weight:bold; margin-left:auto; margin-right:auto">Kembali</a></div>
...
<div class="card" id="kolom-komentar" style="width:95%; margin-left:auto; margin-right:auto">
    ...
    <div class="collapse" id="collapseKomentar">
        ...
        <h5>{{$posts->comment()->count()}} Komentar</h5>
            ...
            @endif
                ...
                @else
                <img class="align-self-start mr-3" src="{{ asset('image/'. $c->user->profile->foto_profil) }}" width="64" height="64"style="border-radius:50%; object-fit: cover;" alt="Generic placeholder image">
                
                @endif
                <div class="media-body" id="komen-{{$c->id}}">
                @if(Auth::user()->id == $c->user_id)
                    <a id="hapus-komentar" data-toggle="modal" data-id="{{$c->id}}" data-target="#hapusKomentar" style="float:right"><button class="btn btn-sm">| hapus komentar</button></a>
                    <a id="edit-komentar" data-toggle="modal" data-id="{{$c->id}}" data-comment="{{$c->comment}}" data-target="#editKomentar" style="float:right"><button class="btn btn-sm">edit komentar</button></a>
                @else
                @endif
                    @if($c->anonymous == 1)
                    <h5 style="font-weight:bold">Anonymous</h4>
                    @else
                    <h5 style="font-weight:bold">{{$c->user->username}}</h5>
                    @endif
                    @if($c->jawaban_terbaik)
    ...
    <!-- modal delete -->
    <div class="modal fade" id="hapusKomentar" data-backdrop="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
    $('.reply-comment-show').click(function(){
        ...
        $.ajax({
            type:"GET",
            url :"/replycomments/getdata/"+rc_id,
            data:"",
            cache:false,
...
<script type="text/javascript">
    $(".btn-reply").click(function(e){
        ...
        success:function(data){
            ...
            $("#collapseExample-"+comment_id).html(data);
            $("#modal-balas #comment").val();
            $("#close-modal-reply").trigger("click");
...
<script type="text/javascript">
    $(document).on('click', '.btn-reply-replied', function(e) {
        ...
        complete: function () {
            $("#btnCloseModal321").trigger("click");
...
<script>
    ...
    $(".btn-delete").click(function(e){
        ...
        success:function(data){
            ...
            $("#kolom-komentar").load(location.href+" #kolom-komentar","");
            },
        error:function(error){
            console.log(error)
        },
...
<script type="text/javascript">
    ...
    $(".btn-edit").click(function(e){
        ...
        success:function(data){
            ...
            $("#komen-"+id).load(location.href+" #komen-"+id,"");
            },
        error:function(error){
            console.log(error)
        },
...
<script>
    $(document).on('click', '#btnHapusBalasKomen', function(e) {
        ...
        $.ajax({
        url:"/replycomment/delete/"+id+"/?"+Math.random(),
        method:'delete',
        data:{
                id:id, 
                },
...
<script type="text/javascript">
    ...
    $(document).on('click', '#btnEditBalasKomen', function(e) {
            ...
            $.ajax({
            url:"/replycomment/edit/"+id+"/?"+Math.random(),
            method:'PUT',
            data:{
                    id:id, 
                    comment:comment,
            ...
            success:function(data){
                alert("sukses");
                $("#daftar-balasan").load(location.href+" #daftar-balasan>*","");
                $("#btnCloseModal3").trigger("click");
                },
            error:function(error){
                console.log(error)
            },
